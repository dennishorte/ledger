...
function pathToNodeId(docsRelPath) {
    // Must end in .md
...
function firstBacktickId(value) {
    return value.match(/`([^`]+)`/)?.[1];
}
/** Extract all backtick-delimited ids from a string. */
function allBacktickIds(value) {
    const ids = [];
...
function findNextH2Start(md, after) {
...
function parseChildrenSection(body) {
    if (/^None\.\s*$/m.test(body.trim()))
        return [];
    const rows = [];
 ...
 */
export function parseDocNode(docsRelPath, raw) {
