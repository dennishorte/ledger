...
- **`.ledger/process/leaf-workflow.md`** — standardised operator playbook for taking a leaf node from PLANNED through COMPLETE. Read before driving a node through its lifecycle.
...
- **Framework playbooks live in `.ledger/process/`** (`leaf-workflow.md`, `maintenance-round.md`, `doc-size-resolution.md`, `decomposition.md`, `verification-signoff.md`) — outside the `docs/` tree entirely, so the parser never touches them. **Underscore-prefixed `docs/` folders are also framework-internal** — the parser (`buildDocGraph`/`parseDocNode`, mirrored in `app/server/serverIdForPath.ts`) skips any docs path containing an `_`-prefixed directory segment, so these never appear in the DAG. `docs/_investigations/` holds incident / root-cause writeups that back an Open Issue (`dispatcher-hang-issue.md`, `e2e-dispatch-findings.md`); `docs/_schemas/` holds machine-readable JSON Schemas. Files here do **not** carry a `**Node ID:**`, `**Parent:**`, or lifecycle `**Status:**` — they are LIVING reference material, not implementation nodes. Add a new internal folder by prefixing its name with `_` (no parser change needed). Issues *of record* still live in the owning node's Open Issues section and the PRD §11 roll-up — `_investigations/` is long-form backing analysis only.
- Round-2 UI panels (`02-dag` (v1.4 — collapsible subtrees; decomposed 2026-06-06 into 4 COMPLETE children: `01-data-source`/`02-layout`/`03-rendering`/`04-inspector`), `03-docs`, `04-tasks`, `05-logs`, `06-health`, `08-markdown`, `09-workflow-progress`, and `10-orchestration` all COMPLETE; `07-replay` DEFERRED in PRD v0.5.1 — out of v1 scope; `99-maintenance/01-round-1` COMPLETE v1 2026-05-26 — first batched maintenance pass closed 5 Open Issues across `02-dag`, `03-docs`, `04-tasks`, `05-logs`, `06-health`). `03-docs` consumes `08-markdown` via the `<MarkdownBody>` contract; `09-workflow-progress` embeds in `02-dag`'s `NodeInspector`; `04-tasks` and `05-logs` consume `10-orchestration`'s data layer; `04-tasks` introduces three soft color tokens (`--color-accent-soft`, `--color-warning-soft`, `--color-danger-soft`) in `globals.css` and a `TaskStatusChip` component, both consumed by `05-logs`; `05-logs` also consumes `08-markdown` for `reasoning` event bodies and `src/lib/docLink.ts` (extracted from `03-docs`'s inline resolver). With `01-ui`'s round-2 manifest complete, focus shifts to the backend (PRD §14, decomposed in v0.5.2). Build order: `02-schema` COMPLETE (v1) + `03-project-metadata` COMPLETE (v1) → `04-api-server` COMPLETE (v1, 2026-05-26 — decomposed into 5 sub-leaves: `01-workspace-conversion` → `02-parser-extraction` → `03-server-package` → `04-cli-launcher` + `05-ui-hook-migration`, all COMPLETE; substrate now includes `@ledger/server` Hono API, `ledger` CLI binary, `useDocGraph` migrated to TanStack Query against `/api/docs`) → **`05-task-runner` COMPLETE (v1, 2026-05-28 — all 5 sub-leaves COMPLETE: `01-store-schema` (v1, 2026-05-27 — `better-sqlite3` store + 3-table schema + transactional migrations runner + typed Store API + `Task`/`LogEvent` migration to `@ledger/parser/runner/` + 3 JSON Schemas in `docs/_schemas/`); `02-scheduler` (v1, 2026-05-27 — `Runner` class with re-entrant tick trampoline + set-intersection conflict primitive + executor registry + `noop` built-in + boot-time orphan recovery; `ProjectContext.runner` wired); `04-api-endpoints` (v1, 2026-05-27 — GET /api/tasks + GET /:id + GET /:id/stream SSE with `Last-Event-ID` resume + POST /api/tasks operator-injection; `runner/events.ts` EventBus + `withPublishing` Store decorator); `03-hitl-gate` (v1, 2026-05-28 — `human_review` executor + `awaitHumanReview` on RunnerHandle + POST /api/tasks/:id/approve + /reject with OCC + optional followUp; rejection rationale dual-write (truncated reason on status_change + full text on kind=error detail event); `humanReviewExecutor` in default registry; restart durability via `recoverOrphans` leaving AWAITING_HUMAN_REVIEW untouched); `05-ui-hook-migration` (v1, 2026-05-28 — additive dual-source `useTaskList`/`useTask`/`useLogStream` with `id.includes(":")` runner-vs-transcript discriminant + `useApproveTask`/`useRejectTask` mutation hooks with response-based `setQueryData` for atomic button unmount (D12 amended in stage-8b loop-back after operator caught a 500–1000 ms flicker) + TaskInspector Approve/Reject buttons gated on runner-emitted ∧ AWAITING_HUMAN_REVIEW with OCC via `dbRowVersion` + Status-reason row surfacing latest `status_change.reason`); in-house TS+SQLite per PRD §5)** → **`06-agent-dispatcher` APPROVED (parent v1 spec, 2026-05-28 — MCP-first integration via `@modelcontextprotocol/sdk` streamable-HTTP transport mounted at `POST /mcp` on the existing Hono app; `ClaudeCodeExecutor` spawns `claude` subprocesses with injected `--mcp-config` + `--prompt-file` + `LEDGER_TASK_ID` env; 5 MCP tools — `runner.emit_event` / `complete_task` / `fail_task` / `await_human_review` / `get_task` — flow agent activity into the runner's events table; transcript ingestion stays additive per D15; decomposed into 5 sub-leaves: `01-mcp-server` → `02-runner-tools` → {`03-claude-code-executor`, `04-prompt-templates`} → `05-dispatch-api`; `01-mcp-server` COMPLETE (v1, 2026-05-28 — `@modelcontextprotocol/sdk@^1.29` (`McpServer` + `WebStandardStreamableHTTPServerTransport`) mounted at `/mcp` via `app.all` since the transport handles GET/POST/DELETE; AsyncLocalStorage carries inbound `Request` into `onsessioninitialized` so `02-runner-tools` can read `X-Ledger-Task-Id` off it; `ProjectContext.mcp: McpServerHandle` wired in `loadProjectContext`; `/api/_health` extended with `dispatcher: { status, activeSessions }`); `02-runner-tools` COMPLETE (v1, 2026-05-28); `03-claude-code-executor` COMPLETE (v1, 2026-05-29 — `execa@^9.6` subprocess wrapper with `claudeBin?` test-only override for fake-claude swap; pure `reconcileExit` lifecycle function — `final === "CANCELLED"` checked first, then exit-code/signal catch-all; cancellation registry on `ProjectContext.dispatchCancellation` for `05-dispatch-api`'s cancel route; 3 prescribed reasons + `executorInternalError` defensive builder; env-gated smoke test; `renderPrompt` was stubbed locally during parallel implementation while 04 worked in a sibling worktree — replaced by the real import at 04's merge); `04-prompt-templates` COMPLETE (v1, 2026-05-29 — eight TS templates + `shared.ts` composition helpers with explicit per-kind LogEvent field schema in the MCP-tool contract reminder; typed `Record<Persona, ...>` registry with `Persona = Exclude<TaskType, "noop" | "human_review" | "operator_session" | "agent_task">`; `defaultResourceClaims(task)` per parent D11; `pathForNodeId` added to `@ledger/parser` as additive cross-package export; `ProjectContext.docs` + `ProjectContext.resolveDocPath` wired in `loadProjectContext`; deterministic snapshot fixtures; 03's `renderPrompt` stub replaced by the real import in the merge commit); `05-dispatch-api` COMPLETE (v1, 2026-05-31 — POST /api/dispatch/:nodeId with status-aware 409 hints; POST /api/tasks/:id/cancel with try/catch on store race per Spec Review B1; `defaultResourceClaims` promoted to `@ledger/parser` per Spec Review S2; useDispatch / useCancelTask mirror useApproveTask's D12-amended setQueryData pattern; NodeInspector Dispatch button gated on `authored ∧ APPROVED/VERIFY/DRAFT` with inline DispatchConfirmDialog; TaskInspector Cancel button gated on `RUNNING ∧ transcriptPath === undefined`; inline error banners replace toast); `06-agent-dispatcher` parent COMPLETE (v1, 2026-06-01 — stage-8 live verification found and fixed 3 bugs: prompt templates used UUID instead of nodeId for doc-path resolution; `--permission-mode dontAsk` + `--allowedTools "mcp__ledger-runner__*"` required to unblock MCP tool calls in `--print --bare` mode; per-persona write grants for implement/spec_draft/doc_refactor; acceptance items 1–4, 6–8, 10 verified live; item 5 partial — no APPROVED leaf in current state; item 9 deferred to routine use))** → **`07-health-daemon` COMPLETE (v2, 2026-06-07 — on-demand health scanner, report-only). v1 (poll-based daemon → enqueue write-agents) was completed 2026-06-01, immediately disabled (`LEDGER_DAEMON_ENABLED=1`), then replaced entirely by v2: the v1 daemon files + `GET /api/daemon/status` + the env gate are deleted. v2 lives in `server/src/scanner/` (`createHealthScanner(ctx).runScan()`), triggered by `POST /api/health/scan`, with `GET /api/health/scans` returning a durable append log (`health_scans` table, migration 002). Three monitors: size (default 12000 tokens), open_issue (stable-state node carrying ≥1 unstruck HIGH/MEDIUM open issue — v2.1, 2026-06-07, replaced the noisy `lastUpdated`-based `orphan` monitor), schema-invalid. The scanner's ONLY persistent write is `store.insertScan` — no `createTask`, no enqueue, no dispatch (the entire point of the redesign — v1's auto-dispatch raced the git index). Config in `.ledger/project.json` `health` key (defaults apply when absent). The DRAFT → COMPLETE reconciliation (2026-06-07) ran the missing clean-context implementation review + live verification (acceptance 1–7, 9 ✅; 8 code-present) and landed review fixes: per-doc monitor-error isolation, scanner test coverage (`server/test/scanner*.test.ts`, server suite 367 → 378), schema default 3000 → 12000, and an unrelated `doc_decompose`/`TaskTypeBadge` app-build break. See `docs/07-health-daemon.md` and `docs/_investigations/e2e-dispatch-findings.md` §2–§3 for v1 history.** Post-manifest, **`08-alerts` COMPLETE (v1, 2026-06-08 — algedonic push channel)** adds a report-only global EventBus observer (`subscribeAll`, additive to the COMPLETE EventBus) that raises an `Alert` on every `RUNNING→FAILED` and pushes it over a webhook (`LEDGER_ALERT_WEBHOOK`) **and** an always-mounted UI banner via `GET /api/alerts/stream`; `Alert` is canonical in `@ledger/parser`; channel wired in `loadProjectContext` as `ctx.alerts`, attached after orphan recovery so restarts don't replay historical failures. Closes the §11 passive-algedonic issue (operator browser sign-off after a controlled live `RUNNING→FAILED` run). **`09-transcript-decouple` DRAFT (2026-06-07)** — decision-enabling spec for the §11 transcript-coupling issue (paths A/B/C, B recommended); awaiting operator path approval before implementation. The original PRD §14 backend manifest (`02`–`07`) is fully COMPLETE. See PRD §14 for the manifest and §11 for open issues.
