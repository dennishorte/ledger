...
- Round-2 UI panels (`02-dag` and `08-markdown` COMPLETE; `03-docs` APPROVED + in implementation; `06-health` DRAFT; `04-tasks`, `05-logs`, `07-replay` planned) are designed for parallel dispatch in isolated git worktrees now that the shell is COMPLETE. `03-docs` depends on `08-markdown` (shared markdown rendering) via the `<MarkdownBody>` contract. See `docs/01-ui/00-ui.md` Children section.
