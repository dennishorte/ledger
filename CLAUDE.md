...
- Round-2 UI panels (`02-dag`, `03-docs`, and `08-markdown` COMPLETE; `06-health` DRAFT; `04-tasks`, `05-logs`, `07-replay` planned) are designed for parallel dispatch in isolated git worktrees now that the shell is COMPLETE. `03-docs` consumes `08-markdown` via the `<MarkdownBody>` contract. See `docs/01-ui/00-ui.md` Children section.
