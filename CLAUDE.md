...
- **`docs/_process/leaf-workflow.md`** — standardised operator playbook for taking a leaf node from PLANNED through COMPLETE. Read before driving a node through its lifecycle.
...
- **Underscore-prefixed folders are framework-internal** — the parser (`buildDocGraph`/`parseDocNode`, mirrored in `app/server/serverIdForPath.ts`) skips any docs path containing an `_`-prefixed directory segment, so these never appear in the DAG. `docs/_process/` holds operator playbooks and process documentation (`leaf-workflow.md`, `maintenance-round.md`, `doc-size-resolution.md`); `docs/_investigations/` holds incident / root-cause writeups that back an Open Issue (`dispatcher-hang-issue.md`, `e2e-dispatch-findings.md`); `docs/_schemas/` holds machine-readable JSON Schemas. Files here do **not** carry a `**Node ID:**`, `**Parent:**`, or lifecycle `**Status:**` — they are LIVING reference material, not implementation nodes. Add a new internal folder by prefixing its name with `_` (no parser change needed). Issues *of record* still live in the owning node's Open Issues section and the PRD §11 roll-up — `_investigations/` is long-form backing analysis only.
