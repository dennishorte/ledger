...
- Round-2 UI panels (`02-dag`, `03-docs`, `04-tasks`, `06-health`, `08-markdown`, `09-workflow-progress`, and `10-orchestration` COMPLETE; `05-logs` VERIFY pending sequential merge; `07-replay` deferred pending doc-versioning) are designed for parallel dispatch in isolated git worktrees now that the shell is COMPLETE. `03-docs` consumes `08-markdown` via the `<MarkdownBody>` contract; `09-workflow-progress` embeds in `02-dag`'s `NodeInspector`; `04-tasks` and `05-logs` consume `10-orchestration`'s data layer; `04-tasks` introduces three soft color tokens (`--color-accent-soft`, `--color-warning-soft`, `--color-danger-soft`) in `globals.css` and a `TaskStatusChip` component, both consumed by `05-logs`; `05-logs` additionally consumes `08-markdown` for `reasoning` event bodies. See `docs/01-ui/00-ui.md` Children section.
