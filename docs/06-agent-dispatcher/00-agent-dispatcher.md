...
|----|-------|------------|--------|
| `01-mcp-server` | MCP server scaffolding mounted at `POST /mcp`: streamable-HTTP transport via `@modelcontextprotocol/sdk`, server factory + Hono route mount, `serverInfo` discovery, internal session lifecycle (open/close hooks for the binding registry from `02-runner-tools`) | `05-task-runner` | VERIFY |
