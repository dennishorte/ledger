...
**Status:** COMPLETE
**Created:** 2026-05-28
**Last Updated:** 2026-06-06 (filed HIGH Open Issue — dispatched agent can hang indefinitely with no executor watchdog; see `docs/_investigations/dispatcher-hang-issue.md`)
...
- **Dispatched agent can hang indefinitely with no watchdog.** A `claude --print --bare` subprocess that freezes — no exit, no telemetry — is never bounded: `spawnClaudeCode` (`server/src/dispatcher/executor/spawn.ts`) calls `execa` with **no `timeout`**, the lifecycle reconciler runs only on subprocess *exit*, and `--print` emits no incremental output. The task is then stuck `RUNNING` forever holding its resource claim, with no way to distinguish a slow run from a frozen one. Hit live on 2026-06-06: a `doc_decompose` dispatch ran 2h23m / ~20 s CPU / 0 events / 0 files before being cancelled by hand; auth (API key returns `200`) and the decompose fix (claim + target verified correct) were both ruled out, leaving a stalled `/mcp` round-trip as the leading suspect (`activeSessions` stayed pinned at 1 throughout, and at 1 *after* the kill — a session leak). Distinct from the "Zombie subprocesses after eager cancel" issue above: that is post-cancel SIGTERM-resistance; this is a pre-cancel freeze that nothing detects. Minimum fix is an executor `timeout` → kill + `FAILED:subprocess_timeout`; better is an idle/heartbeat watchdog plus `--output-format stream-json` telemetry; also needed are hung-process reconciliation and MCP session teardown on subprocess death. The watchdog is independent of the underlying stall and should land regardless — nothing should be able to hang the runner indefinitely. Full writeup, repro, ranked hypotheses, and acceptance criteria: `docs/_investigations/dispatcher-hang-issue.md`. *(Priority: HIGH — blocks reliable end-to-end dispatch; the runner cannot currently run an agent to completion or surface why. Both `doc_decompose` runs to date produced zero model output.)*
