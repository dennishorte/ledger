...
**Status:** COMPLETE
**Created:** 2026-05-28
**Last Updated:** 2026-06-06 (resolved the HIGH dispatcher-hang Open Issue in `345dfaa` — per-session MCP transports + watchdog + stream-json telemetry; filed its LOW residual, the MCP turn-0 startup race, as a separate Open Issue; see `docs/_investigations/dispatcher-hang-issue.md` §Resolution)
...
- **Subscription-auth path for dispatched `claude` subprocesses.** D17 selects `--bare` and locks auth to `ANTHROPIC_API_KEY` (API-format only). Operator's `claude setup-token` mints an OAuth token (`sk-ant-oat01-…`) tied to a Pro/Max subscription, which **cannot** be supplied via the env var (`--bare` rejects it as "Invalid API key"). Stage-8 verification on 2026-06-01 prototyped removing `--bare` to allow claude's native keychain auth path; the prototype proved the keychain credential works for a trivial prompt but reintroduced the determinism costs D17 was designed to avoid (CLAUDE.md auto-discovery, hooks, plugin sync, background prefetches), AND required a defensive env-strip for `sk-ant-oat`-prefixed `ANTHROPIC_API_KEY` values to prevent the operator's `.env`-pasted OAuth token from poisoning the inherited subprocess env. Operator chose to defer this in favour of the simpler API-credit path (Path B in `.env.example`). The investigation surfaces a real product gap — subscription users currently can't dispatch without paying for API credits — but the cost-benefit of solving it (drop `--bare`, ship the env-strip, accept context double-load, document the keychain dependency) didn't clear the bar at v1. Reopen when one of: (a) subscription billing becomes a real adoption blocker, (b) `claude --bare` adds an OAuth-token path, (c) a per-dispatch `apiKeyHelper` script that mints API-format keys from a subscription becomes available. The trade-off analysis lives in this Open Issue rather than as a stage-8b ISSUE_OPEN loop because the v1 path (API key) works and is well-documented; subscription support is genuinely additive scope. *(Priority: LOW — deferred at operator preference; no v1 user impact since the API-key path is the documented one.)*
- **Dispatched agent can miss MCP tools on turn 0 (claude `--print` startup race).** claude's `--print` `init` event reports `mcp_servers:[{status:"pending"}]` for the first ~hundreds of ms while the MCP client is still completing its handshake, so a *single-shot* prompt that does its whole job in turn 0 and exits can run before the `runner.*` tools are bound — it then cannot `emit_event` or issue a terminal call, and a clean exit reconciles to `FAILED:subprocess_exit_without_terminal_status`. Independent of and not fixed by the per-session-transport work (`345dfaa`), which only removed the deterministic "first agent per boot binds, the rest get `-32600`" cause; this is a claude-side timing race that survives it. Scope is narrow and no longer silent: real multi-turn dispatch agents connect within a turn or two (a probe reached the tool by ~turn 3), and stream-json telemetry forwarding now makes a turn-0 miss visible in the Logs UI rather than a black box. Candidate mitigations (pick when it bites): a prompt-level directive to call a cheap `runner.*` tool such as `get_task` before doing any work, forcing the connection on turn 0; a short executor-side readiness wait (poll `mcp_servers` status / the handle's session map) before handing the prompt to the agent; or a bounded retry. Surfaced as the residual when the dispatcher hang was resolved — `docs/_investigations/dispatcher-hang-issue.md` §Residual. *(Priority: LOW — only single-shot prompts are exposed; multi-turn agents are unaffected and the failure is now observable rather than silent.)*
- ~~**Dispatched agent can hang indefinitely with no watchdog.**~~ — **RESOLVED 2026-06-06 (`345dfaa`).** Root cause was a single shared MCP transport: the SDK rejected every `initialize` after the first with `-32600 "Server already initialized"`, so only the first dispatched agent per server boot loaded `runner.*` tools; later ones ran blind, emitted no telemetry, and — with no executor `timeout` — could hang forever holding their resource claim. Fix: **per-session transports** (`Map<sessionId, …>`, a fresh transport + `McpServer` per `initialize`, routed by `mcp-session-id`, torn down on close) + `closeTaskSessions` on subprocess exit (defect #4); execa **`timeout`** → `FAILED:subprocess_timeout` plus a stream-json **idle watchdog** → `FAILED:subprocess_idle` (defects #1/#3, the reconciler now forces a hung process to exit); **`--output-format stream-json`** event forwarding so a run is an inspectable transcript rather than a black box (defect #2). Live-verified: two agents both connect (was 400 on the second), a real `verify` dispatch produced 77 events, `activeSessions` returned to 0, and the short-cap run was killed with `subprocess_timeout`. Residual: a narrow claude-side turn-0 startup race can still make a single-shot prompt miss tools — now tracked as its own LOW Open Issue above ("Dispatched agent can miss MCP tools on turn 0"). Full writeup, repro, and verification: `docs/_investigations/dispatcher-hang-issue.md` §Resolution. *(Was HIGH.)*
...
- ~~`--mcp-config` flag JSON shape (`"type": "http"` vs `"type": "sse"` vs `"type": "streamable-http"`) — pin the exact format.~~ → Verified by `06-agent-dispatcher/99-maintenance/02-round-2` (2026-06-12): `claude mcp add --transport http` CLI flag directly corresponds to `"type": "http"` in the JSON config; confirmed in `claude 2.1.172` help output. No code change to `mcpConfig.ts`.
...
| `03-claude-code-executor` | `ClaudeCodeExecutor` registered for the eight real task types; subprocess spawn via `execa@^9.6` with `--print --bare --mcp-config <path>` (D17) and prompt piped via stdin (D16); `LEDGER_TASK_ID` env; pure `reconcileExit` lifecycle function (Spec Review B2 — `final === "CANCELLED"` honoured first, then exit-code/signal catch-all); new status reasons `SUBPROCESS_EXIT_WITHOUT_TERMINAL_STATUS` (bare const) + `subprocessFailed(tail)` (builder; 80-char truncation) + `CANCELLED_BY_OPERATOR` (bare const) + `executorInternalError(msg)` (builder; defensive for pre-spawn failures) registered in `runner/scheduler.ts` reasons const; cancellation registry `Map<TaskId, Subprocess>` exposed on `ProjectContext.dispatchCancellation` for `05-dispatch-api`'s cancel route; fake-claude fixture as primary integration test (`spawnClaudeCode.claudeBin?` test-only override per Spec Review B3); env-gated smoke test via `LEDGER_SMOKE_TESTS` | `02-runner-tools` | COMPLETE (v1) |
| `04-prompt-templates` | Eight per-task-type TS prompt templates (`implement`, `spec_review`, `verify`, `spec_draft`, `reverify`, `doc_refactor`, `issue_triage`, `project_status_review`) plus a `shared.ts` helper (persona preamble, MCP-tool contract reminder with explicit per-kind required fields, required-reading composition, `taskHeaderBlock`); template registry in `prompts/index.ts` typed `Record<Persona, ...>` for compile-time exhaustiveness; `defaultResourceClaims(task)` per type (D11; all bullets discriminant-tagged `{ kind: "node", ... }`); `pathForNodeId` added to `@ledger/parser` as additive cross-package export (Spec Review S3 — the index did not previously exist); deterministic snapshot fixtures (Spec Review S2) | `02-runner-tools` | COMPLETE (v1) |
| `05-dispatch-api` | **Dispatch + cancel endpoints + UI integration.** `POST /api/dispatch/:nodeId` with lifecycle-driven task-type inference (APPROVED → implement, VERIFY → verify, DRAFT → spec_review; status-aware 409 hints per Spec Review S1); `POST /api/tasks/:id/cancel` with eager-CANCELLED transition + SIGTERM (D14) + `cancelled_by_operator` reason + try/catch on store race (Spec Review B1); `useDispatch` / `useCancelTask` mutation hooks (the latter mirrors `useApproveTask`'s D12-amended setQueryData pattern); `NodeInspector` Dispatch button (visibility on `authored ∧ APPROVED/VERIFY/DRAFT`); `TaskInspector` Cancel button (visibility on RUNNING ∧ runner-emitted via `transcriptPath === undefined`); `defaultResourceClaims` promoted to `@ledger/parser` (Spec Review S2 — eliminates client/server mirror + drift-test) | `03-claude-code-executor`, `04-prompt-templates` | COMPLETE (v1) |

| `99-maintenance` | Maintenance container — holds rounds of post-COMPLETE issue resolution; permanent `APPROVED` operating state | — | APPROVED |
| `99-maintenance/01-round-1` | SIGKILL escalation after SIGTERM on cancel — `killWithEscalation` + escalation timer in `cancellation.ts`; `subprocess_killed` LogEvent kind; doc-strike cross-refs in `03`/`05`/parent | — | COMPLETE (v1, 2026-06-12) |
