...
**Status:** VERIFY
**Created:** 2026-05-28
**Last Updated:** 2026-06-01 (VERIFY-stage amendment — D17 reversed: `--bare` dropped from subprocess invocation after live testing revealed `--bare`'s strict-API-key requirement was incompatible with `claude setup-token`'s OAuth-format output. Trade-off accepted by operator.)
...
| D17 (added 2026-05-28; reversed 2026-06-01 stage-8 verification) | Subprocess invocation is `claude --print --mcp-config <path>` — **`--bare` dropped** | **`--print`** is mandatory: without it Claude Code drops into an interactive TUI and never exits, defeating the executor's purpose. **`--bare` reversal rationale (2026-06-01):** the original 2026-05-28 amendment selected `--bare` to skip hooks/plugin sync/attribution/auto-memory/keychain/CLAUDE.md auto-discovery + to lock auth to `ANTHROPIC_API_KEY` strictly. Stage-8 verification on 2026-06-01 surfaced a real-world incompatibility: the operator's preferred auth path (`claude setup-token`, which mints an OAuth-style `sk-ant-oat01-…` token tied to their Pro/Max subscription) was hard-rejected by `--bare`'s strict-API-key check (claude printed `Invalid API key · Fix external API key` to stdout — captured by our `--print` consumption and visible only because the operator ran claude manually). The two ways forward were: (a) get a separate `sk-ant-api03-…` key from the API console (uses API credits, not subscription billing), or (b) drop `--bare` so claude's normal auth path (OAuth/keychain) is restored. Operator chose (b) to keep subscription billing. Trade-offs reintroduced: CLAUDE.md auto-discovery (the dispatched subprocess re-loads CLAUDE.md on top of the prompt-template-injected context — duplicated context, not corrupted), hooks fire (configured at the operator's `.claude/settings.json`), plugin sync runs, background prefetches run. The determinism cost is real but bounded; operator accepts it for auth ergonomics. Spec amendment lands at parent VERIFY stage; `03-claude-code-executor` stays COMPLETE with a deviation note in its Implementation Notes referencing this entry. |
