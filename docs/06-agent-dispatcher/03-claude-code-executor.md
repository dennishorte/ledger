...
- ~~**`subprocess_failed:` reason captures stderr only — but `claude --print` writes user-visible errors (including auth failures) to STDOUT.** Surfaced during parent `06-agent-dispatcher`'s stage-8 operator verification on 2026-06-01: an auth misconfiguration produced `Invalid API key · Fix external API key` on the subprocess's stdout — captured by our `--print` consumption — but the executor's `result.stderr ?? ""` resolved to empty, so the operator saw `subprocess_failed:` with no diagnostic. Fix: change `lifecycle.ts`'s catch-all to `reasons.subprocessFailed(result.stderr || result.stdout || "")` (preferring stderr, falling back to stdout). One-line edit; 80-char truncation in the reason builder still applies. The eventual structured-event store (preceding bullet) would carry both streams unambiguously. *(Priority: MEDIUM — silent-failure UX bug; the captured-but-discarded stdout has been the operator's first concrete diagnosis blocker.)*~~ → struck 2026-06-12: fix already shipped — `lifecycle.ts:92` uses `result.stderr || result.stdout || ""`, verified against the live file; confirmed implemented during the 2026-06-01 parent `06-agent-dispatcher` stage-8 verification.
