...
- **`subprocess_failed:` reason captures stderr only — but `claude --print` writes user-visible errors (including auth failures) to STDOUT.** Discovered during parent `06-agent-dispatcher`'s stage-8 operator verification on 2026-06-01: an auth misconfiguration (subscription OAuth token supplied where `--bare` expected an API key) produced `Invalid API key · Fix external API key` on the subprocess's stdout — captured by our `--print` consumption, but the executor's `result.stderr ?? ""` resolved to empty, so the operator saw `subprocess_failed:` with no diagnostic. Fix: change `lifecycle.ts`'s catch-all to `reasons.subprocessFailed(result.stderr || result.stdout || "")` (preferring stderr, falling back to stdout). One-line edit; 80-char truncation in the reason builder still applies. The eventual structured-event store (preceding bullet) would carry both streams unambiguously. *(Priority: MEDIUM — silent-failure UX bug; the captured-but-discarded stdout has been the operator's first concrete diagnosis blocker.)*
...
### Post-COMPLETE deviation (2026-06-01)

During parent `06-agent-dispatcher` VERIFY-stage operator verification (parent §Acceptance check items 3–6) on 2026-06-01, two real-world gaps surfaced that motivated post-COMPLETE code changes to this leaf without re-entering the lifecycle (the leaf stays COMPLETE; the parent's D17 amendment commit carries the changes). Documented here so future readers see code-vs-spec mismatch noted explicitly per the "doc and code must agree" rule:

1. **`--bare` dropped from `spawn.ts`'s claude argv.** Parent D17 originally selected `--bare` for context-determinism + strict-API-key auth, but `--bare` hard-rejected the operator's preferred subscription-OAuth token (`claude setup-token` mints a `sk-ant-oat01-…` format that `--bare` cannot accept). Operator chose to reverse `--bare` to keep subscription billing. Parent D17 amended on 2026-06-01 with the full trade-off rationale; this leaf's spec references to `--bare` (D6, D7, D11 ordering, §Requirements item 5's `claude --print --bare --mcp-config <path>` pseudocode, etc.) are now superseded by parent D17's reversal. `spawn.ts`'s argv is now `[...prefixArgs, "--print", "--mcp-config", path]`; `spawn.test.ts`'s argv assertion now positively asserts `--bare` is NOT present.
2. **New Open Issue logged: subprocess_failed reason captures stderr only.** Discovered during the same stage-8 verification: claude's `Invalid API key · Fix external API key` auth error went to stdout (via `--print`) and was not surfaced in our `subprocess_failed:` reason because the executor only reads `result.stderr`. The Open Issue in §Open Issues above documents the proposed fix (`result.stderr || result.stdout || ""`) and the priority bump (LOW → MEDIUM — silent-failure UX bug). Not patched in this commit; carried as backlog.
