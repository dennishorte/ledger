...
**Created:** 2026-05-28
**Last Updated:** 2026-06-06 (D12 added — status-aware `doc_decompose`, after the first live decompose; prior: 2026-05-29 VERIFY → COMPLETE — implementation review applied, snapshots locked rendering correctness mechanically)
...
| D12 | Status-aware `doc_decompose`: children **inherit the target's lifecycle status** (COMPLETE/VERIFY → same; DRAFT/APPROVED/PLANNED → PLANNED), an **IN_PROGRESS target is refused**, and a **terminal target (COMPLETE/VERIFY) routes the split through `runner.await_human_review`** instead of `runner.complete_task`. (`doc_decompose` is the 9th persona, added post-v1 in `c75cedc`; this Decision extends it.) **Mode A parent-status decision (2026-06-12):** When an APPROVED/DRAFT/PLANNED node is forward-decomposed into PLANNED children, the parent's status is unchanged — APPROVED continues to mean "approved to implement this subtree," now as a decomposed set of leaves. No intermediate state is introduced. Decision recorded in PRD §6.2 by `06-agent-dispatcher/99-maintenance/02-round-2`. | The original template hardcoded PLANNED children + "do not change any status," assuming decomposition is always *forward* planning. But the size trigger fires on COMPLETE docs too — every oversized doc in this tree is COMPLETE — and a COMPLETE parent cannot own PLANNED children (a parent is COMPLETE iff its children are done): that breaks the parent⊇children status invariant. Two modes fall out: **(A) forward** — split unbuilt scope into PLANNED child specs; **(B) retroactive** — split shipped work into children that inherit the parent's terminal status and describe the *actual* implementation (not stubs), landing via `human_review` because the agent is re-describing code it did not write and can misrepresent it. IN_PROGRESS is refused to avoid forking a spec out from under a running implementer (the runner's claim intersection would serialize it, but decomposing mid-implementation is wrong regardless). Caught live 2026-06-06: a decompose of COMPLETE `01-ui/02-dag` emitted four PLANNED children for already-shipped subsystems; the operator's manual review corrected it — Mode B's `human_review` routing makes that gate automatic. |
...
- ~~**MCP tool contract reminder is fixed-string; future tool additions in `02-runner-tools` require manual reminder update.**~~ → Build-time assertion added: `server/test/dispatcher/prompts/shared.test.ts` now includes a `RUNNER_TOOLS` array test that verifies `mcpToolContractReminder()` mentions all four canonical tool names. Future tool additions to the reminder must also extend `RUNNER_TOOLS`. (`06-agent-dispatcher/99-maintenance/02-round-2`, 2026-06-12.) *(Was Priority: TRIVIAL.)*
...
- ~~**Parent status on forward (Mode A) decompose is undecided (D12 follow-up).**~~ → Decision recorded in PRD §6.2 by `06-agent-dispatcher/99-maintenance/02-round-2` (2026-06-12): APPROVED parent keeps APPROVED after forward decompose. No new lifecycle state. Closes the Mode A open issue. *(Was Priority: MEDIUM.)*
