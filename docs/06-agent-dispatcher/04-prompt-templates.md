...
- **Parent status on forward (Mode A) decompose is undecided (D12 follow-up).** When an APPROVED node (approved to implement as one unit) is decomposed into PLANNED children, the parent is no longer "ready to implement directly" — it is now a coordinator awaiting child work — yet D12 keeps the target's status unchanged. The lifecycle (§6.2) has no explicit "parent of planned children" state. For now the parent keeps its pre-decompose status; revisit if a forward decompose of an APPROVED/DRAFT node produces a misleading parent status. Mode B (the COMPLETE/VERIFY case) is unaffected — a COMPLETE parent of COMPLETE children is consistent. *(Priority: MEDIUM — only bites Mode A, which has not been exercised live yet.)*
