...
- **Daemon disabled at runtime pending safety fixes (2026-06-01).** See the banner at the top of this doc. The first e2e dispatch test exposed two HIGH defects — uncontrolled write-agent dispatch racing the git index, and starved enqueues — that make the daemon unsafe to run unattended. It now requires `LEDGER_DAEMON_ENABLED=1` to start. **Chosen direction is a report-only redesign (v2) — see "Proposed redesign" above:** the daemon stops dispatching write-agents entirely and instead produces a durable, deduplicated report of issues; remediation becomes an explicit operator decision. This dissolves findings #2 and #3 rather than hardening the write path. *(Priority: HIGH — blocks unattended operation. Tracked in `docs/00-project.md` §11 and `docs/process/e2e-dispatch-findings.md` §2–§3. A v2 leaf to resolve the durability/dedup model is to be drafted.)*
...
## Proposed redesign (v2 — report-only, durable, deduplicated)

Direction set 2026-06-02. The v1 model ("detect → enqueue a remediation task → dispatch a write-agent") is being abandoned — it caused findings #2 and #3. v2 reframes the daemon as a **read-only health reporter**.

**1. Report, don't remediate.** The daemon must **not** enqueue `doc_refactor` / `reverify` / `issue_triage` tasks, and must **not** dispatch any agent that mutates docs. It only *detects and records* issues. This eliminates the write-agent git-index race (finding #2) and the dispatch-starvation problem (finding #3) outright — there are no runner tasks to schedule at all. Remediation becomes an explicit operator decision: the operator reads the report and dispatches a fix if they want one. This supersedes PRD §6.4's "enqueues an appropriate task." (PRD §6.5's `doc_refactor` protocol and task type are **not** removed — a human may still dispatch one deliberately; v2 only removes the *daemon's* authority to trigger writes automatically.)

**2. Durable reports.** Findings must persist across restarts. v1's `DaemonStatus.lastFindings` is in-memory and resets to zero on boot — useless as a standing health record. A finding gets a lifecycle (open → resolved) with first-seen / last-seen timestamps and retained history, backed by a small persistent store (a `health_findings` table, or a committed report artifact).

**3. Deduplication — the hard problem (OPEN).** A finding needs a **stable identity** so the same condition isn't re-reported every tick or on every trivial edit ("flagging minor changes to the same thing over and over"). Coupled with durability — you can't dedup without a durable identity, and you don't want a durable list that grows unboundedly or nags. Unresolved design questions to work through:
- **Identity / fingerprint** — key a finding as `{monitor, nodeId, class}`, not a per-tick row; re-detection updates `lastSeen` on the existing finding instead of creating a new one.
- **Materiality / hysteresis** — only re-surface or re-open when the metric moves beyond a *band* (e.g. size crossing a higher threshold), so it doesn't flap around the boundary on ±a-few-tokens edits.
- **Acknowledgement / snooze** — the operator can ack or mute a finding; an acked finding stays suppressed unless the condition *materially worsens*. Requires persisted ack state keyed by finding identity.
- **Resolution** — a finding auto-closes when the condition clears, and the closure is recorded so "fixed then regressed" is visible rather than re-appearing as brand-new.

This section is a direction + open questions, not a settled spec. A v2 leaf should be drafted to resolve the dedup/durability model before any re-enable.
