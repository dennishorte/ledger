...
- **LOW — `args` object passed to `Workflow({ name: ... })` is not deserialized; scripts receive it as a string.** When the Workflow tool is invoked with a named workflow and an `args` object (e.g. `{ nodeId: "12-..." }`), the framework delivers `args` to the script as the raw JSON string rather than a parsed object. `args.nodeId` is therefore `undefined` and guard clauses like `if (!nodeId) return ...` fire immediately with 0 agents run. Workaround: edit the copied script under the session's `workflows/scripts/` directory to replace `args.nodeId` with a hardcoded default (or `JSON.parse`-style coercion), then re-invoke via `scriptPath`. The `args` global works correctly when it is a plain string (single-value case). Fix: the scripts should defensively coerce `args` — `const parsed = typeof args === 'string' ? JSON.parse(args) : (args ?? {})` — so both delivery modes work. Observed on both `leaf-workflow.js` and `leaf-workflow-finish.js` during `12-workflow-decomposition` (2026-06-12).
