...
- ~~**LOW — `args` object passed to `Workflow({ name: ... })` is not deserialized; scripts receive it as a JSON string.**~~ **FIXED (2026-06-12):** Both scripts now coerce `args` via `typeof args === 'string' ? JSON.parse(args) : (args ?? {})` before reading any property, so both string-delivery and object-delivery modes work.
