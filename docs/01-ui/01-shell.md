...
- **`StatusBar` shows hardcoded "offline" indicator.** `src/components/layout/StatusBar.tsx:18` renders `<span>offline</span>` with a faint dot regardless of any connection state — visible on every page in the global footer. Spec D §Navigation model names the status bar as the "connection-state indicator" surface, but no source feeds it. Either wire it to a real connection signal (TanStack Query online status, or `useLogStream`-style EventSource health), or drop the slot until there's something to show. Surfaced during `99-maintenance/01-round-1` verification (2026-05-26). *(Priority: LOW — visible nit; defer to a future maintenance round.)*
