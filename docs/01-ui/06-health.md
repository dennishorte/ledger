...
- Each row: left-aligned `StatusChip`-style node badge (reuses `StatusChip` from `src/components/ui/`), priority badge (`HIGH` in `--color-danger`, `MEDIUM` in `--color-warning`, `LOW`/`TRIVIAL` in `--color-muted`), and the issue text truncated to 2 lines. Full text on hover (native `title` attribute) or in the shell inspector.
...
| D7 | Reuse `StatusChip` from `src/components/ui/` as-is | At authoring time the chip lived under `dag/` and the call was to keep it there with two consumers. `99-maintenance/01-round-1` R4 (2026-05-26) moved it to `src/components/ui/` once the third consumer landed; this row updated to match. |
