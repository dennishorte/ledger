...
- ~~**Remaining UI consumers not migrated** — `useDocSource`, `useHealthData`'s remaining surface, the orchestration hooks. Each is its own follow-up commit per PRD §7.2. The orchestration hooks specifically wait for `05-task-runner`. Inherited from parent. *(Priority: MEDIUM for the docs hooks, LOW for orchestration.)*~~ Resolved by `04-api-server/99-maintenance/01-ui-hook-migration` (COMPLETE 2026-06-12) — `useDocSource` and `useHealthData` both migrated to TanStack Query against live API.
