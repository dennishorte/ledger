...
| D2 | Root `package.json` carries **no runtime dependencies**, only workspace-level scripts. **Exception (added 2026-05-26 by `04-api-server/04-cli-launcher`):** packages that declare a `bin` entry may be listed at the root so pnpm wires `node_modules/.bin/<bin>` for workspace-wide invocation via `pnpm exec <bin>`. `@ledger/server` at the root enables `pnpm exec ledger <project-path>`. | Every dep lives where it's used. Hoisting deps to the root is technically possible (pnpm does it transparently for shared transitives) but declaring deps at the root invites "what owns this?" confusion — typescript at the root would be a candidate for "I'll just use this from anywhere," which breaks encapsulation. Each package declaring its own deps keeps the boundaries explicit. The bin-wiring carve-out is honest about pnpm's actual semantics: `node_modules/.bin/<bin>` only gets a shim when a consuming package declares the dep, and the workspace root is the right consumer for a binary intended to be workspace-wide. |
