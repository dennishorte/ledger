...
- ~~**`reasons.rejected(rationale)` truncates at 80 chars; UI must render the full rationale from the detail event.** Drift risk: if a future UI surface reads `event.reason` instead of the detail event's `stack`, the operator sees only 80 chars. The `05-ui-hook-migration` spec should explicitly call out the detail-event read path. Logged here so the `05` author sees it. *(Priority: MEDIUM — cross-leaf coupling.)*~~ → **RESOLVED** in `05-task-runner/99-maintenance/01-hitl-rejection-rationale-ui-display` (2026-06-12): explicit protective comment added at the `latestStatusReason` usage site in `TaskInspector.tsx`; test coverage added confirming only the truncated form appears in the Status reason row.
