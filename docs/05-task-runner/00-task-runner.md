...
| `04-api-endpoints` | Read endpoints (`GET /api/tasks`, `/:id`, `/:id/stream` SSE with `Last-Event-ID` resume), operator-injection endpoint (`POST /api/tasks`) with `TaskInput` validation against the schema, in-process pub/sub bridge (`runner/events.ts`) closing `02-scheduler`'s pub/sub Open Issue | `02-scheduler` | SPEC_REVIEW |
