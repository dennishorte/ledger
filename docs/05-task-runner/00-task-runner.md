...
| `03-hitl-gate` | `human_review` executor + suspension semantics (`RUNNING → AWAITING_HUMAN_REVIEW`, claims held), `POST /api/tasks/:id/approve` + `/reject` endpoints with rationale capture and optional follow-up enqueue, restart durability for suspended tasks | `02-scheduler`, `04-api-endpoints` | IN_PROGRESS |
