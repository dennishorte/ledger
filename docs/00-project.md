...
- **MCP tools load intermittently in dispatched agents; failure silently corrupts task status.** When the `runner.*` MCP toolset fails to bind in a session, the agent still does its work (and may `git commit` it) but cannot `emit_event` or issue a terminal call — so the executor reconciles the clean exit to `FAILED` (`subprocess_exit_without_terminal_status`). Task status then no longer reflects outcome. Observed twice in the first e2e dispatch test (a `doc_refactor` that committed real work yet shows FAILED). Suspected concurrency / large-prompt binding race in `onsessioninitialized`. *(Priority: HIGH — breaks the runner's premise that status reflects reality. Full detail + repro in `docs/_investigations/e2e-dispatch-findings.md` §1.)*
