...
| `10-e2e-testing` | Playwright browser-driven acceptance suite — `e2e/` workspace package, `webServer` boots full stack, tests for every COMPLETE UI panel and primary interaction flows (dispatch, HITL approve/reject, health scan, alert banner) | `04-api-server`, `01-ui` | VERIFY (2026-06-09) |
| `11-workflow-scripts` | Workflow scripts — two `.claude/workflows/` JS scripts automating the leaf-workflow playbook (stages 1–8 in `leaf-workflow.js`; stages 9–11 in `leaf-workflow-finish.js`); idempotent re-entry; structured review output via sign-off matrix | `.ledger/process/leaf-workflow.md`, `05-task-runner/04-api-endpoints` | IN_PROGRESS (2026-06-10) |
