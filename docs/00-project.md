...
**`doc_decompose` — Mode A forward decompose (parent status):** When a `doc_decompose` task runs against an APPROVED, DRAFT, or PLANNED node and creates new child nodes (Mode A — forward decompose), the template sets the new children to PLANNED status; the **parent's status is unchanged**. APPROVED continues to mean "approved to implement this subtree," now as a decomposed set of leaves rather than a single leaf. The operator re-approves each child individually. No intermediate parent status is introduced. (Decision recorded by `06-agent-dispatcher/99-maintenance/02-round-2`, 2026-06-12.)
...
- **No autonomous System 4 (intelligence / adaptation).** 🔴 The framework has strong S1/S2/S3/S5 (operations, coordination, control, policy) but no autonomous component that scans the *external* environment the agents depend on — dependency/CLI/SDK drift, upstream API changes. All "what to build next / has the environment shifted" cognition is the human operator plus the static §14 manifest. The project's own incident history is the evidence: every dispatch break was external drift nothing watched for (`tsx watch` dropping `ANTHROPIC_API_KEY`; `claude` flags needing `--permission-mode dontAsk` + `--allowedTools`; the MCP single-transport behaviour), each caught by a human only after it broke. This is acceptable and deliberate for v1 (human-as-S4), but it is the ceiling on the project's stated autonomy ambition — the system can run a plan, it cannot re-plan itself. Any "less operator-in-the-loop" roadmap is an S4-construction roadmap. The push/alert half of this is now built — `08-alerts` (the algedonic channel, resolved below) gives a report-only path that interrupts the operator. The remaining first step is adjacent to the now-COMPLETE health scanner: feed it genuinely **external** scan targets (dependency/CLI/SDK drift) and route findings through the existing `08-alerts` push path, without ever regaining write authority over operations (the v1 daemon's fatal mistake). Backing analysis: `docs/_investigations/vsm-diagnosis.md` finding 1. *(Priority: HIGH — strategic; frames the autonomy roadmap, see §13.)*
- ~~**Passive algedonic channel — failures are logged, never pushed.**~~ — *Resolved by `08-alerts` (COMPLETE v1, 2026-06-08; VSM finding 3). A report-only global EventBus observer raises an `Alert` on every `RUNNING→FAILED` transition and pushes it over two paths: an outbound webhook (`LEDGER_ALERT_WEBHOOK`) that reaches an away operator, and an always-mounted UI banner fed by `GET /api/alerts/stream` that surfaces on every route. The channel holds the same observe-only discipline as the v2 scanner (no store writes, no dispatch). The push primitive is built; wiring genuinely-**external** scan targets (dependency/CLI/SDK drift, VSM finding 1) into it remains open under the S4 issue below. Detail: `docs/08-alerts.md`.*
- ~~**Self-audit problem**~~ — *Both named deliverables resolved (v0.5.10, 2026-06-07).* **(1) Independent reviewer persona (the *who*)** shipped at `06-agent-dispatcher` COMPLETE: `spec_review` / `verify` / `reverify` dispatch as separate clean-context `claude` subprocesses — the author no longer checks its own work. **(2) Structured per-requirement sign-off format (the *rigor*)** now lives in `docs/_process/verification-signoff.md`, wired into those three prompt templates and leaf-workflow stages 2 & 6: one row per Requirements/Acceptance/coverage item, PASS/FAIL/PARTIAL/N/A with evidence, a PASS-without-evidence recorded as FAIL, headline verdict derivable from the matrix. VSM diagnosis framed this as an S3\*/S4 independence + requisite-variety failure (`docs/_investigations/vsm-diagnosis.md` finding 4). **Residual (LOW):** the format shapes a review's *output* but nothing *enforces* that an independent review actually runs before COMPLETE — an operator can still skip the gate (the 07-health-daemon reconciliation was exactly that: v2 shipped, reviewed only after the fact). True enforcement is a task-runner COMPLETE-transition gate that refuses without a recorded sign-off matrix; deferred until dispatch is the default path rather than the manual exception. *(Was HIGH, blocked verification design; residual LOW.)*
...
- **Transcript ingestion couples the orchestration data layer to one agent runtime.** `01-ui/10-orchestration` parses Claude Code transcript JSONL. This is a bootstrap, not a target architecture; the eventual MCP-based dispatch (§5) emits a different shape. **Now specced as `09-transcript-decouple` (DRAFT, 2026-06-07)** — a decision-enabling spec posing three resolution paths (A drop the source / B adapter boundary / C runner-native ingestion), B recommended; awaiting operator approval of the path in its D0 before implementation. *(Priority: MEDIUM — revisit when the task runner ships and dispatch becomes the data source; the runner shipped, so the node is now actionable.)*
...
| `08-alerts` | Algedonic alert channel — pushes critical task failures to the operator over two paths (webhook + always-mounted UI banner) off a report-only global EventBus observer; closes the §11 passive-algedonic issue (VSM finding 3) | `05-task-runner`, `01-ui/01-shell` | COMPLETE (v1, 2026-06-08 — implemented + independent clean-context review (READY_WITH_FOLLOWUPS, all R1–R6/D1–D7 PASS, F1/F2/F3/N3 fixes applied); server suite 381 → 401; operator browser sign-off after a controlled live `RUNNING→FAILED` run) |
| `09-transcript-decouple` | Decouple the orchestration data layer from the Claude Code transcript format (§11 issue; §7.2 migration seam) — behind a runtime-agnostic ingestion-adapter boundary | `06-agent-dispatcher`, `01-ui/10-orchestration` | DRAFT (2026-06-07 — decision-enabling spec; awaiting operator approval of path A/B/C in D0 before implementation) |
| `10-e2e-testing` | Playwright browser-driven acceptance suite — `e2e/` workspace package, `webServer` boots full stack, tests for every COMPLETE UI panel and primary interaction flows (dispatch, HITL approve/reject, health scan, alert banner) | `04-api-server`, `01-ui` | COMPLETE (v1, 2026-06-09 — 22 pass / 1 skip; `pnpm test` fanout confirmed across all 4 workspace packages) |
| `11-workflow-scripts` | Workflow scripts — two `.claude/workflows/` JS scripts automating the leaf-workflow playbook (stages 1–8 in `leaf-workflow.js`; stages 9–11 in `leaf-workflow-finish.js`); idempotent re-entry; structured review output via sign-off matrix | `.ledger/process/leaf-workflow.md`, `05-task-runner/04-api-endpoints` | COMPLETE (v1, 2026-06-10) |
| `12-workflow-decomposition` | Decomposition checkpoint — adds a break-if-yes signal list at the end of leaf-workflow stage 1 so operators exit to decomposition.md before submitting an oversized or multi-responsibility DRAFT for spec review; doc-only change, no new lifecycle state | `.ledger/process/leaf-workflow.md`, `.ledger/process/decomposition.md`, `docs/00-project.md` §6.6 | COMPLETE (v1, 2026-06-12) |
