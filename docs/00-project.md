...
| `12-workflow-decomposition` | Decomposition checkpoint — adds a break-if-yes signal list at the end of leaf-workflow stage 1 so operators exit to decomposition.md before submitting an oversized or multi-responsibility DRAFT for spec review; doc-only change, no new lifecycle state | `.ledger/process/leaf-workflow.md`, `.ledger/process/decomposition.md`, `docs/00-project.md` §6.6 | DRAFT |
