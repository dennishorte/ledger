...
> **Superseded (2026-06-02).** The "enqueue a remediation task" model above caused the daemon to auto-dispatch unreviewed write-agents (see §11). The chosen direction is **report-only**: the daemon detects and records issues in a durable, deduplicated report, and remediation becomes an explicit operator decision rather than an automatic dispatch. The `doc_refactor` task type (§6.5) survives as an operator-initiated action; the daemon simply loses the authority to trigger it. See `docs/07-health-daemon.md` §"Proposed redesign (v2)".
...
- **Health daemon auto-dispatches unreviewed write-agents that race the shared git index.** Discovered in the first end-to-end dispatch test (2026-06-01). Booting the server in a project with a valid `ANTHROPIC_API_KEY` is sufficient to make the health daemon (§6.4) enqueue `doc_refactor` tasks against *every* spec exceeding `LEDGER_DAEMON_SIZE_THRESHOLD_TOKENS` (default 3000 — most real specs exceed it). `doc_refactor` is a write-persona, so each task spawns a `claude` agent that edits a spec **and runs `git add`/`git commit` directly on the working branch, with no HITL gate.** Two distinct defects: **(1) No operator gate on daemon-originated write tasks** — autonomous, unprompted spec rewrites + commits the moment the server boots. **(2) The resource-claim model (§6.3) protects logical node writes but not the shared git index/working tree** — distinct-node `doc_refactor` tasks carry non-conflicting claims and are scheduled concurrently, but they all contend for the repo-global git index lock. In the test, ~24 agents fired; 6 won the commit race, ~18 lost it (their file edits survived in the working tree but their commits were dropped), one stuck half-`git add`ed. *Chosen direction (2026-06-02): **report-only redesign.*** Rather than hardening the write path (isolated worktrees, HITL gates, git-index claims), the daemon stops dispatching write-agents altogether — it produces a **durable, deduplicated report** of issues and leaves remediation to an explicit operator decision. This dissolves both this issue and the starvation issue below. The open sub-problem is durability + dedup (don't re-flag the same condition on every tick / minor edit) — see `docs/07-health-daemon.md` §"Proposed redesign (v2 — report-only, durable, deduplicated)". *(Priority: HIGH — daemon disabled until the v2 leaf lands. Note: the worktree-isolation + `human_review` + git-index-claim ideas remain the right model for **operator-initiated** write-agents generally, just not for the daemon.)*
- **MCP tools load intermittently in dispatched agents; failure silently corrupts task status.** When the `runner.*` MCP toolset fails to bind in a session, the agent still does its work (and may `git commit` it) but cannot `emit_event` or issue a terminal call — so the executor reconciles the clean exit to `FAILED` (`subprocess_exit_without_terminal_status`). Task status then no longer reflects outcome. Observed twice in the first e2e dispatch test (a `doc_refactor` that committed real work yet shows FAILED). Suspected concurrency / large-prompt binding race in `onsessioninitialized`. *(Priority: HIGH — breaks the runner's premise that status reflects reality. Full detail + repro in `docs/process/e2e-dispatch-findings.md` §1.)*
- **Daemon-enqueued tasks starve — never dispatched without an external scheduler tick.** The daemon enqueues via raw `store.createTask`, the scheduler has no self-timer, and it only ticks on `runner.createTask`/`runner.tick`. Daemon tasks sit `PENDING` until incidental operator activity ticks the scheduler — so the daemon is not actually autonomous. *(Priority: HIGH, but **dissolved by the report-only redesign** — a report-only daemon enqueues no runner tasks, so there is nothing to starve. Retained as a record of the v1 defect and as a live concern for any other producer that enqueues via raw `store.createTask`. Detail in `docs/process/e2e-dispatch-findings.md` §3.)*
...
| `07-health-daemon` | Document health daemon — size, staleness, orphan-issue monitors; enqueues `doc_refactor` / `reverify` / `issue_triage` tasks (§6.4) | `06-agent-dispatcher` | COMPLETE (v1, 2026-06-01) — **DISABLED at runtime; v1 enqueue-remediation model superseded by a report-only redesign (v2), see `docs/07-health-daemon.md`** |
