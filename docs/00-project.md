...
**Status:** IN_PROGRESS (`07-health-daemon` v2 redesign in DRAFT)
**Version:** 0.5.5
...
> **Superseded (2026-06-03 — v2 design).** The "enqueue a remediation task" model above caused the daemon to auto-dispatch unreviewed write-agents (see §11). v2 replaces the poll-based daemon entirely with an **on-demand health scanner**: operator triggers a scan via `POST /api/health/scan`; the scanner runs four monitors (size, staleness, orphan, schema-invalid), appends findings to a durable `health_scans` log, and does nothing else — no task enqueue, no agent dispatch. The `doc_refactor` task type (§6.5) survives as an operator-initiated action. See `docs/07-health-daemon.md`.
...
- ~~**Health daemon auto-dispatches unreviewed write-agents that race the shared git index.**~~ — *Dissolved (2026-06-03) by the v2 redesign: `07-health-daemon` is now an on-demand scanner with no write authority beyond its own findings log — it enqueues no tasks and dispatches no agents. Note: worktree-isolation + `human_review` + git-index-claim remain the right model for operator-initiated write-agents generally. Detail: `docs/process/e2e-dispatch-findings.md` §2.*
...
- ~~**Daemon-enqueued tasks starve.**~~ — *Dissolved (2026-06-03): v2 scanner enqueues no runner tasks — nothing to starve. Detail: `docs/process/e2e-dispatch-findings.md` §3.*
...
| `07-health-daemon` | Document health scanner — on-demand scan via `POST /api/health/scan`; four monitors (size, staleness, orphan, schema-invalid); durable append log; report-only, no task enqueue (§6.4 v2) | `06-agent-dispatcher` | DRAFT (v2 redesign, 2026-06-03 — v1 complete but disabled; v2 spec in progress) |
