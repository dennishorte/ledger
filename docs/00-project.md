...
| `06-agent-dispatcher` | Agent dispatcher — MCP-based interface; Claude Code as first integration; transcript ingestion stays additive (D15) — full retirement deferred to a future node | `05-task-runner` | VERIFY (2026-05-28 — parent spec drafted, reviewed in clean Sonnet context, applied 2 blocking + 4 should-fix + 4 nit findings; post-APPROVAL amendments: `claude --help` corrections (256e2b6) + scope-conformance for `02-runner-tools`'s schema-file deferral; 5-child manifest: `01-mcp-server` COMPLETE (v1, 2026-05-28), `02-runner-tools` COMPLETE (v1, 2026-05-28), `03-claude-code-executor` COMPLETE (v1, 2026-05-29 — execa subprocess wrapper, pure `reconcileExit` lifecycle, cancellation registry, fake-claude integration fixture, env-gated smoke; `renderPrompt` stubbed during parallel impl, replaced at 04's merge), `04-prompt-templates` COMPLETE (v1, 2026-05-29 — eight TS templates + shared composition helpers, typed `Record<Persona, ...>` registry, `defaultResourceClaims` per D11, new `pathForNodeId` parser export, `ProjectContext.docs` + `resolveDocPath` wired, deterministic snapshots, 03's renderPrompt stub replaced), `05-dispatch-api` COMPLETE (v1, 2026-05-31 — dispatch + cancel endpoints + 2 mutation hooks + Dispatch/Cancel buttons; `defaultResourceClaims` promoted to `@ledger/parser` per Spec Review S2); parent `06-agent-dispatcher` now VERIFY (all 5 children COMPLETE); sequencing `01` → `02` → {`03`, `04`} → `05`; MCP-first via streamable-HTTP transport mounted at `POST /mcp` on existing Hono app) |
