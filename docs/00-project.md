...
| `08-alerts` | Algedonic alert channel — pushes critical task failures to the operator over two paths (webhook + always-mounted UI banner) off a report-only global EventBus observer; closes the §11 passive-algedonic issue (VSM finding 3) | `05-task-runner`, `01-ui/01-shell` | VERIFY (2026-06-07 — implemented + independent clean-context review (READY_WITH_FOLLOWUPS, all R1–R6/D1–D7 PASS); server suite 381 → 401; awaiting operator browser walk of acceptance 1–6 for COMPLETE) |
