...
- **Daemon-enqueued tasks starve — never dispatched without an external scheduler tick.** The daemon enqueues via raw `store.createTask`, the scheduler has no self-timer, and it only ticks on `runner.createTask`/`runner.tick`. Daemon tasks sit `PENDING` until incidental operator activity ticks the scheduler — so the daemon is not actually autonomous. *(Priority: HIGH — the daemon must drive the runner or the scheduler needs a periodic tick. Detail in `docs/process/e2e-dispatch-findings.md` §3.)*
...
| `07-health-daemon` | Document health daemon — size, staleness, orphan-issue monitors; enqueues `doc_refactor` / `reverify` / `issue_triage` tasks (§6.4) | `06-agent-dispatcher` | COMPLETE (v1, 2026-06-01) |
