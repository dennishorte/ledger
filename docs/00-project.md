...
| `06-agent-dispatcher` | Agent dispatcher — MCP-based interface; Claude Code as first integration; replaces `10-orchestration`'s transcript ingestion as the data source | `05-task-runner` | SPEC_REVIEW (2026-05-28 — parent spec authored with 5-child manifest: `01-mcp-server`, `02-runner-tools`, `03-claude-code-executor`, `04-prompt-templates`, `05-dispatch-api`; MCP-first integration shape, transcript ingestion stays additive per operator preference) |
