...
> **Superseded (2026-06-03 — v2 design).** The "enqueue a remediation task" model above caused the daemon to auto-dispatch unreviewed write-agents (see §11). v2 replaces the poll-based daemon entirely with an **on-demand health scanner**: operator triggers a scan via `POST /api/health/scan`; the scanner runs three monitors (size, orphan, schema-invalid — staleness was dropped during implementation as it fired on doc-sync commits with no actionable signal), appends findings to a durable `health_scans` log, and does nothing else — no task enqueue, no agent dispatch. The `doc_refactor` task type (§6.5) survives as an operator-initiated action. v2 shipped COMPLETE 2026-06-07. See `docs/07-health-daemon.md`.
...
- **Decomposition termination criteria** — Need explicit rules for when a node is too small to decompose further (minimum task size, depth limit, complexity threshold). Without this, recursive decomposition produces unnavigable trees. VSM diagnosis names this the "recursion documented, not operational below the leaf" gap — a dispatched agent that spawns sub-work is not modeled as its own viable system (`docs/_investigations/vsm-diagnosis.md` finding 5). *(Priority: MEDIUM)*
...
| `07-health-daemon` | Document health scanner — on-demand scan via `POST /api/health/scan`; three monitors (size, orphan, schema-invalid); durable append log; report-only, no task enqueue (§6.4 v2) | `06-agent-dispatcher` | COMPLETE (v2, 2026-06-07 — on-demand scanner; v1 daemon deleted. DRAFT → COMPLETE reconciliation ran the missing clean-context review + live verification (acceptance 1–7, 9 ✅; 8 code-present) and landed review fixes: per-doc error isolation, scanner test coverage (367 → 378), schema default 3000 → 12000, and an unrelated `TaskTypeBadge`/`doc_decompose` app-build regression. v1 was complete-but-disabled 2026-06-01.) |
