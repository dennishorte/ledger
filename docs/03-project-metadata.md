...
| D7 | Build-time validation via direct Vite JSON import (no fetch, no `import.meta.glob`) | Mirrors `02-schema`'s D6 — same posture (validate at build time, no runtime fetch). Mechanism: a direct `import x from "...json" with { type: "json" }` for both the schema artifact and `.ledger/project.json`. `import.meta.glob` was the DRAFT's proposed mechanism but spec review B1 surfaced that a literal path with no wildcard silently matches nothing; direct import is the idiomatic Vite pattern for a known-singleton file (and matches `02-schema`'s own schema-import line in `validateDocNode.ts`). The artifact is small (~100 bytes), static, and present at build time; runtime fetch adds complexity (loading state, error UI) for no gain. When the API server lands, validation moves server-side, same migration path as `02-schema`. |
...
Independent implementation review was run against the rebased worktree in a clean Sonnet context. Verdict: READY_FOR_OPERATOR_VERIFICATION (two should-fixes, one nit, three operator notes — no code-quality blockers). All eight Spec Review audit closures verified honored in code (B1 direct JSON import at `loadProjectMetadata.ts:11`; B2 topbar pseudocode at `Topbar.tsx:24–29`; S1 both missing-field fixtures present; S2 `ValidationError` re-exported, not redeclared; S3 schema description carries the path-traversal handoff; N1/N2/N3 stale references resolved). Audit:

| # | Finding | Resolution |
|---|---------|------------|
| F1 | D7's title and rationale still named `import.meta.glob` as the chosen mechanism, contradicting the Design section and D10's own reference to "D7's chosen mechanism" (direct JSON import). Stale DRAFT text not updated when spec review B1 reversed the decision. | Rewrote D7 title → "Build-time validation via direct Vite JSON import (no fetch, no `import.meta.glob`)". Rationale now walks through the B1 reversal explicitly so a future reader of D7 in isolation sees the actual mechanism + the reason `import.meta.glob` was rejected, and references the precedent in `02-schema`'s `validateDocNode.ts`. |
| F2 | Verification item 10 still mentioned `import.meta.glob` in the "loader is pure build-time + browser-safe" line. Stale text from the same B1 reversal. | Rewrote to "direct JSON import + ajv". |
| F3 | `loadProjectMetadata()` JSDoc claimed the function was "exported for test use," but tests do not import it (they import `validateProjectMetadata` and `projectMetadata`). Inaccurate documentation. | Rewrote JSDoc: function is a thin wrapper around `validateProjectMetadata` exposed for future callers (e.g. the API server); production UI uses the `projectMetadata` singleton below. |
| Op-1 | `CLAUDE.md` round-2 line still shows `03-project-metadata (APPROVED)`. | No worktree change — per leaf-workflow stage 10, CLAUDE.md cross-doc sync bundles into the merge commit on main. Confirmed correct sequencing. |
| Op-2 | Bundle delta is +12.52 kB gzip (vs. spec estimate of ~1–3 kB). Cause is real (second `new Ajv2020()` instance), explanation is correct, and the overhead drops naturally when validation moves server-side with `04-api-server`. Sharing an ajv instance across `validateDocNode.ts` and `loadProjectMetadata.ts` via a third module would recover ~11 kB but adds an extraction step. | Logged as Open Issue (see below). Deferred per the implementer's stated rationale: both instances drop at `04-api-server`. |
| Op-3 | `loadProjectMetadata()` and `validateProjectMetadata()` could appear redundant to a future reader. They are not — `validateProjectMetadata(unknown)` is the testable validator and `loadProjectMetadata()` is the wrapper that binds `rawProject` from the Vite-imported JSON. Design is sound. | F3's JSDoc rewrite now explains the split. No further action. |

Re-ran gates after F1 + F2 + F3 edits (F1/F2 are doc-only; F3 touches only a comment):

- `pnpm -C app typecheck` → exit 0
- `pnpm -C app lint --max-warnings=0` → exit 0
- `pnpm -C app test` → exit 0 (118 tests, 6 files — unchanged)
- `pnpm -C app build` → exit 0

Added Open Issue for Op-2 (shared ajv instance). All other findings are mechanical and now closed. Audit table stays as durable provenance.
...
10. No Node-only imports leak into the browser bundle; the loader is pure build-time + browser-safe (direct JSON import + ajv, both already used in the browser by `02-schema`).
