...
| `04-cli-launcher` | `ledger` CLI binary at `server/src/bin/ledger.ts` exposed via the package's `bin` field; `parseArgs` strict + try/catch + `Number.isInteger` port guard; headless-safe `open(url)` wrap; SIGINT graceful shutdown; CLI tests via spawned subprocess | `03-server-package` | SPEC_REVIEW |
| `05-ui-hook-migration` | Migrate `useDocGraph` to TanStack Query against `/api/docs` with `placeholderData: () => loadDocNodes()` build-time fallback; add Vite dev proxy `server.proxy: { "/api": "http://127.0.0.1:4180" }`; add mocked-fetch hook test; close `03-project-metadata`'s "docs path validation" Open Issue with a pointer here | `03-server-package` | SPEC_REVIEW |
