...
|----|-------|------------|--------|
| `01-workspace-conversion` | Convert repo to pnpm workspace — root `package.json` + `pnpm-workspace.yaml` declaring `app`, `server`, `packages/*`; rename `app` package to `@ledger/app`; no source code moves | — | APPROVED |
