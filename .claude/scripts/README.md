...
|---|---|---|
| `sync-process [<path> ...]` | manual per-project cp | push `.ledger/process/` updates to all registered child projects (or named ones). Reports `ok` (already current) vs `sync` (updated). |
